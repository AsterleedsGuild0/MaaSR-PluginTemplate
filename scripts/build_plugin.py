...
def build_plugin(repo_root: Path, output_dir: Path, skip_deps: bool = False) -> Path:
    """构建插件
    ...
    Returns:
        Path: 插件目录路径
    """
    # 删除旧的根目录 plugin.json，确保从 pyproject.toml 读取最新配置
    root_plugin_json = repo_root / "plugin.json"
    if root_plugin_json.exists():
        log_info(f"删除旧的配置文件: {root_plugin_json}")
        root_plugin_json.unlink()
...
def create_zip_package(plugin_dir: Path, output_dir: Path, version: str) -> Path:
    ...
    # 删除旧的 ZIP 文件
    if zip_path.exists():
        log_info(f"删除旧的 ZIP 文件: {zip_path}")
        zip_path.unlink()
...
def main() -> int:
    ...
    parser.add_argument(
        "--output", type=Path, default=Path("dist"), help="输出目录 (默认: dist/)"
    )
